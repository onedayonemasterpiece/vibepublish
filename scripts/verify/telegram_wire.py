...
def cases():
    ...
    reply_to = t.InputReplyToMessage(reply_to_msg_id=3, top_msg_id=3)
    ...
    values = {
        ...
        "replies": (f.messages.GetRepliesRequest, dict(peer=peer, msg_id=3, offset_id=0, offset_date=None, add_offset=0, limit=10, max_id=0, min_id=0, hash=0)),
        "forum_topics": (f.messages.GetForumTopicsByIDRequest, dict(peer=peer, topics=[3])),
        ...
        "send_text": (f.messages.SendMessageRequest, dict(peer=peer, message=TEXT, random_id=11, entities=entities, schedule_date=at, reply_to=reply_to)),
        "send_media": (f.messages.SendMediaRequest, dict(peer=peer, media=media, message=TEXT, random_id=11, entities=entities, schedule_date=at, reply_to=reply_to)),
        "send_album": (f.messages.SendMultiMediaRequest, dict(peer=peer, multi_media=album, schedule_date=at, reply_to=reply_to)),
...
def verify(*, core: bool = False) -> dict:
    ...
    if core:
        ...
        input_file = t.InputFile(id=6, parts=1, name='fixture.png', md5_checksum='0'*32)
        filename = compiler.type('DocumentAttributeFilename', file_name='fixture.png')
        uploaded_document = compiler.type('InputMediaUploadedDocument', file=input_file, mime_type='image/png',
                                          attributes=[filename], force_file=True)
        input_document = compiler.type('InputDocument', id=7, access_hash=8, file_reference=b'fixture')
        document_media = compiler.type('InputMediaDocument', id=input_document)
        topic_reply = compiler.type('InputReplyToMessage', reply_to_msg_id=3, top_msg_id=3)
        for value in (filename, uploaded_document, input_document, document_media, topic_reply):
            roundtrip(value)
